...
impl Vertex for ModelVertex {
    fn desc<'a>() -> wgpu::VertexBufferLayout<'a> {
        ...
        wgpu::VertexBufferLayout {
            array_stride: mem::size_of::<ModelTextureVertex>() as wgpu::BufferAddress,
...
#[repr(C)]
#[derive(Clone, Copy, Debug, bytemuck::Pod, bytemuck::Zeroable)]
pub struct ModelTextureVertex {
    pub position: [f32; 3],
    pub tex_coords: [f32; 2],
    pub normal: [f32; 3],
}

impl Vertex for ModelTextureVertex {
    fn desc<'a>() -> wgpu::VertexBufferLayout<'a> {
        use std::mem;
        wgpu::VertexBufferLayout {
            array_stride: mem::size_of::<ModelTextureVertex>() as wgpu::BufferAddress,
...
impl Model {
    pub fn load<P: AsRef<Path>>(
        ...
        // We assume the texture files are in the same folder with the .obj file
        ...
        if with_materials {
            for mat in obj_materials {
                // *Diffuse
                let diffuse_path = mat.diffuse_texture.clone();
        ...
        for model in obj_models {
            if with_materials {
                let mut vertices = Vec::<ModelTextureVertex>::new();

                // println!("{}", model.mesh.positions.len());
                // println!("{}", model.mesh.texcoords.len());
                // println!("{}", model.mesh.normals.len());
                println!("{}", model.name);
                // if model.name == "Light_Square_Tall_Solid_Arrow_Point" {
                //     let text = std::fs::read_to_string(path.as_ref()).expect("File not found!");

                //     let mut lines = text.lines();

                //     let st = lines
                //         .find(|x| *x == "o Light_Square_Tall_Solid_Arrow_Point")
                //         .unwrap();

                //     let arrow_pos = lines.next().unwrap();
                //     let splited_pos: Vec<&str> = arrow_pos.split(" ").collect();
                //     for i in 1..4 {
                //         arrow_points.push(splited_pos[i].parse::<f32>().unwrap());
                //     }
                //     println!("{:?}", arrow_points);
                // }

                for i in 0..model.mesh.positions.len() / 3 {
                    vertices.push(ModelTextureVertex {
                        position: [
                ...
                // *To calculate tangents and bitangets.
                // *Use the triangles, so loop through the indices in chunks of 3
    ...
    fn draw_model_instanced(
        ...
        for mesh in &model.meshes {
            if model.materials.len() > 0 {
                ...
                println!("<<< Model without Material >>>");
                self.set_vertex_buffer(0, mesh.vertex_buffer.slice(..));
                self.set_index_buffer(mesh.index_buffer.slice(..), wgpu::IndexFormat::Uint32);
